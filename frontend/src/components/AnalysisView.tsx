...
export function AnalysisView({ analysisId }: { analysisId: string }) {
  ...
  if (!analysis) {
    return (
      <div className="space-y-4">
        <div className="h-28 animate-shimmer rounded-xl bg-ink-100" />
        <div className="h-64 animate-shimmer rounded-xl bg-ink-100" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <nav className="flex min-w-0 items-center gap-2 text-xs text-ink-500">
        <Link href="/" className="shrink-0 hover:text-ink-900 hover:underline">
          Analyse
        </Link>
      ...
      {failed && analysis.error && (
        <section className="card card-pad border-red-200 bg-red-50">
          <h2 className="text-sm font-semibold text-red-900">The analysis could not complete</h2>
          <p className="mt-1.5 text-sm leading-relaxed text-red-800">{analysis.error}</p>
